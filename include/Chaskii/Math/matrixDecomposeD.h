...
#define caseMacroD(i,j,k,I,J,K) \
	...
	break
			caseMacroD(x,y,z,X,Y,Z);
			caseMacroD(y,z,x,Y,Z,X);
			caseMacroD(z,x,y,Z,X,Y);
			}
		}
	...
	{
#define sgnD(n,v)    ((n)?-(v):(v))
#define swapVecD(a,i,j) {a[3]=a[i]; a[i]=a[j]; a[j]=a[3];}
#define cycleD(a,p)  if (p) {a[3]=a[0]; a[0]=a[1]; a[1]=a[2]; a[2]=a[3];}\
		...
		if (turn>=0) 
        {
			...
			case X: q = Qt_Mul(q, qtoz = qxtoz); swapVecD(ka,X,Z) break;
			case Y: q = Qt_Mul(q, qtoz = qytoz); swapVecD(ka,Y,Z) break;
			...
			case 1: if (neg[1]) p = qppmm; else p = qpppp; cycleD(ka,0) break;
			case 2: if (neg[2]) p = qmpmm; else p = qpppm; cycleD(ka,1) break;
			}
			...
			if (all>two) {
				if (all>big) {/*all*/
					{int i; for (i=0; i<4; i++) pa[i] = sgnD(neg[i], 0.5);}
					cycleD(ka,par);
				}
				else {/*big*/ pa[hi] = sgnD(neg[hi],1.0);}
			} else {
				if (two>big) { /*two*/
					pa[hi] = sgnD(neg[hi],SQRTHALF);
					pa[lo] = sgnD(neg[lo], SQRTHALF);
					if (lo>hi) {
						hi ^= lo; lo ^= hi; hi ^= lo;
					}
					if (hi==W) {
...
inline void matrix4D::decompose(iiMath::vec3D& t,
							iiMath::vec4D& r,
							iiMath::vec3D& s,
							iiMath::vec4D& so )
{
	MatrixDecompositionD md;
	MatrixDecompositionD::_affineParts parts;
	MatrixDecompositionD::_HMatrix 	 hmatrix;
    ...
    double mul = 1.0;
    if (parts.t[MatrixDecompositionD::W] != 0.0)
        mul = 1.0 / parts.t[MatrixDecompositionD::W];
